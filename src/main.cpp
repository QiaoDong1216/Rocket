...
void setup() {
    ...
    while (file_temp) {
        if (file_temp.isDirectory())
            dirs.emplace_back(file_temp.name());
        else
    ...
    T_detach = strtod(T_DetachValue, nullptr);
    T_para = strtod(T_ParaValue, nullptr);
    H_para = strtod(H_ParaValue, nullptr);
    T_protectPara = strtod(T_ProtectValue, nullptr);
    rgbBrightness = strtol(RGB_BrightnessValue, nullptr, 10);
    ...
    if (String(RunModeValue) == "Standard") {
        ...
        case DETACHED:
            ...
            if (String(ParaModeValue) == "height control") {
                ...
                if (((millis() - time_launch) > (T_protectPara * 1000)) && !sign_parachute) {
